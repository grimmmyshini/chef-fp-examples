...
#define INPUT_FILE_NAME "data/overhead.txt"
...
static void BlkSol(benchmark::State &state)
{
...
static void BlkSolWithClad(benchmark::State &state)
{
    // clad::estimate_error(BlkSchlsEqEuroNoDiv);
    FILE *file;
    int i;
...
BENCHMARK(BlkSol)->Unit(benchmark::kSecond);
BENCHMARK(BlkSolWithClad)->Unit(benchmark::kSecond);
