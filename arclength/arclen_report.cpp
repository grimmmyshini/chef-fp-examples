/* 
 * This benchmark outputs only the final error report calculated by clad.
 * (See line 122 for the process involved in generating the report) 
...
/*
 * Singleton class MaxError keeps track of the max intermediate error
 * (stored in error_map variable) and the max final error (stored in
 * total_error_map varible). This is then used by print_report function
 * (at line 170) to print the stored error values per variable. 
 * 
 */
...
public:
    ...
    void set_error(std::string &&var, double err) {
        auto entry = error_map.find(var);

        if (entry == error_map.end() || entry->second < err) {
            error_map[var] = err;
        }
    }

    void set_total_error(std::string &&var, double err) {
        auto entry = total_error_map.find(var);

        if (entry == total_error_map.end() || entry->second < err) {
            total_error_map[var] = err;
        }
    }

    const std::unordered_map<std::string, double>& get_error_map() const {
        return error_map;
    }

    const std::unordered_map<std::string, double>& get_total_error_map() const {
...
private:
    MaxError() {}
    std::unordered_map<std::string, double> error_map;
    std::unordered_map<std::string, double> total_error_map;
};

void print_report() {
    const auto &error_map = MaxError::getInstance().get_error_map();
    for (const auto &error : error_map) {
        std::cout << "Max Error of " << error.first << " = " << error.second << std::endl;
    }

    const auto &total_error_map = MaxError::getInstance().get_total_error_map();
    for (const auto &error : total_error_map) {
        std::cout << "Total Error of " << error.first << " = " << error.second << std::endl;
    }
}
...
void fun_clad_pullback(double x, double _d_y, clad::array_ref<double> _d_x, double &_final_error, std::ostream &_error_stream) {
  ...
  _label0:
        ...
        {
            {
                ...
                * _d_x += _r7;
                ...
                _delta_t30 += std::abs(_r_d3 * (_r9 - (float)_r9));
                MaxError::getInstance().set_error("t30", std::abs(_r_d3 * (_r9 - (float)_r9)));
                _d_t3 -= _r_d3;
            }
            {
                double _r_d2 = _d_d2;
                ...
                _delta_d20 += std::abs(_r_d2 * (_r2 - (float)_r2));
                MaxError::getInstance().set_error("d20", std::abs(_r_d2 * (_r2 - (float)_r2)));
                _d_d2 -= _r_d2;
            }
        }
    }
    * _d_x += _d_t3;
    {
        _delta_d20 += std::abs(_d_d2 * (_EERepl_d200 - (float)_EERepl_d200));
        MaxError::getInstance().set_error("d20", std::abs(_d_d2 * (_EERepl_d200 - (float)_EERepl_d200)));
    }
    double _delta_x = 0;
    _delta_x += std::abs(* _d_x * (x  - (float)x));
    MaxError::getInstance().set_error("x", std::abs(* _d_x * (x  - (float)x)));
    _final_error += _delta_x + _delta_t30 + _delta_d20;
    MaxError::getInstance().set_total_error("x", _delta_x);
    MaxError::getInstance().set_total_error("t30", _delta_t30);
    MaxError::getInstance().set_total_error("d20", _delta_d20);
}
  ...
  _label0:
    * _d_s1 += 1;
        ...
        }
        {
            ...
            }
            {
                ...
                * _d_t1 += -_r10;
                ...
                _delta_s1 += std::abs(_r_d2 * (_r11 - (float)_r11));
                MaxError::getInstance().set_error("s1", std::abs(_r_d2 * (_r11 - (float)_r11)));
                * _d_s1 -= _r_d2;
                * _d_s1;
            }
            {
                ...
                _delta_t20 += _t5;
                MaxError::getInstance().set_error("t20", _t5);
                _d_t2 -= _r_d1;
            }
        }
    }
    {
        ...
        _delta_h = std::abs(_d_h * (h0 - (float)h0));
    }
    _delta_s1 += std::abs(* _d_s1 * (_EERepl_s10 - (float)_EERepl_s10));
    MaxError::getInstance().set_error("s1", std::abs(* _d_s1 * (_EERepl_s10 - (float)_EERepl_s10)));
    _delta_t1 += std::abs(* _d_t1 * (t1 - (float)t1));
    MaxError::getInstance().set_error("t1", std::abs(* _d_t1 * (t1 - (float)t1)));
    _final_error += _delta_t1 + _delta_s1 + _delta_t20;
    MaxError::getInstance().set_total_error("t1", _delta_t1);
    MaxError::getInstance().set_total_error("s1", _delta_s1);
    MaxError::getInstance().set_total_error("t20", _delta_t20);
}
